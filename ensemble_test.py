...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, default="dfg_cp",
    ...
    modelC= rgcn.rgcnNet(num_tasks = dataset.num_tasks, num_layer = args.num_layer, emb_dim = args.emb_dim, drop_ratio = args.drop_ratio).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if test_loss<=np.min(np.array(test_curve)):
            PATH='ensemble_model/'+args.dataset + '_pna_sage_'+ str(args.num_layer)+'epoch_'+str(args.epochs)+'+model.pt'
    ...
    if not os.path.exists('ensemble_result'):
        os.makedirs('ensemble_result')

    f = open('ensemble_result/'+args.dataset + '_pna_sage_classifier'+str(args.num_layer)+'.json', 'w')
